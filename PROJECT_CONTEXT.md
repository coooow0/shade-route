...
- 검색과 결과를 두 화면 상태로 나눈다. 경로를 찾은 뒤에는 검색 폼과 날씨를 숨기고 결과에 집중한다.
- 결과 상단에 출발지·도착지, 추가 시간·절약한 햇빛, 출발 시각을 표시한다.
- 세 경로는 세로 카드 대신 가로 탭형 카드로 압축한다.
- Leaflet과 OpenStreetMap 타일로 실제 지도 배경을 표시한다.
- 출발지와 도착지 마커, 경로 범례, OSM 저작자 표시를 제공한다.
- 실시간 현재 위치는 파란 점과 정확도 원으로 표시하고, `지도 자동 이동`을 켜거나 끈다.
- 상세 도보 안내는 기본으로 접어 지도 아래 스크롤을 줄인다.
...
- 상단에 `출발지 → 도착지` 요약과 `수정` 버튼을 둔다.
- 지도는 화면 높이의 약 40~50%를 차지한다.
- 빠른길, 균형, 그늘우선은 탭 또는 가로 카드로 비교한다.
- `2분 더 걸리지만 햇빛을 8분 덜 받아요` 같은 추천 근거를 먼저 보여준다.
- 상세 길 안내는 기본으로 접고 사용자가 원할 때 펼친다.

현재 코드에 이 흐름을 구현했다. 결과 화면의 남은 스크롤은 상세 안내와 주석·한계 표시를 위한 것이다.
