...
- 모바일에서는 시트 핸들을 아래로 내려 세 경로 비교표와 출발 시각까지만 남길 수 있고, 위로 올리거나 핸들을 누르면 상세 정보를 다시 펼친다.
- Leaflet과 OpenStreetMap 타일로 실제 지도 배경을 표시한다.
- 출발지와 도착지 마커, 경로 범례, OSM 저작자 표시를 제공한다.
- 실시간 현재 위치는 파란 점과 정확도 원으로 표시하고, `지도 자동 이동`을 켜거나 끈다. 관련 버튼은 확대·축소 컨트롤과 겹치지 않도록 지도 좌측 상단에 둔다.
...
- 지도가 남은 뷰포트를 가득 채운다. 지도의 실시간 위치 토글은 확대·축소 컨트롤과 겹치지 않도록 좌측 상단에 둔다.
- 하단 바텀 시트에 빠른길·균형·그늘우선을 4행 지표 표(시간·거리·햇빛·예상 그늘)로 나란히 비교한다. 컬럼 헤더가 모드 선택 탭을 겸한다.
- 시트 핸들을 아래로 드래그하면 지도와 세 경로 비교표·출발 시각만 남기고, 위로 드래그하거나 핸들을 누르면 상세 경로를 복원한다.
