...
경로 비교표 바로 아래 위젯은 처음에 만족도(좋음/보통/나쁨)만 보여준다. 사용자가 하나를 고르면 선택 메모와 요청 도시 입력, `보내기` 버튼을 열고, 명시적으로 전송한 내용만 웹훅으로 보낸다. 좌표·검색어·계정 정보는 보내지 않는다. 이 문서는 Google Apps Script + Google Sheets로 웹훅을 세팅하는 절차다.
